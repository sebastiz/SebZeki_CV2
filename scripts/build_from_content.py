...
def read_publications() -> list[dict]:
    ...
    for folder in sorted(pub_dir.glob("orcid_*")):
        ...
        # Parse citation count from content body (written by update_from_orcid.py)
        citation_match = re.search(r'Cited (\d+) time', text)
...
CV_CSS = """
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  @media print {
    ...
    .cv-section-category { page-break-before: always; }
    .cv-section-allpubs-header { page-break-before: always; }
  }
"""
...
def generate_cv_html(about: dict, experience: list[dict],
    ...
    def _pub_entry(p: dict) -> str:
        ...
        doi = _h(p.get("doi", ""))
        altmetric = (f'<div class="altmetric-embed" data-badge-type="donut" '
                     f'data-badge-popover="right" data-doi="{doi}" '
                     f'style="float:right; margin-left:1rem;"></div>'
                     if doi else "")
        cites = p.get("citation_count")
        cite_badge = (f'<span style="font-size:8pt; color:#555; background:#f0f4f8; '
                      f'padding:1px 5px; border-radius:3px; margin-left:0.5rem;">'
                      f'📖 {cites} citation{"s" if cites != 1 else ""}</span>'
                      if cites is not None else "")
        return f"""
        <div class="pub-entry" style="overflow:hidden;">
          {altmetric}
          <div class="pub-title">{doi_link}</div>
          <div class="pub-authors">{_author_str(p)}</div>
          <div class="pub-meta">{pub_display}{('. ' + year) if year and year not in p['publication'] else ''}{cite_badge}</div>
        </div>"""
    ...
    def _pub_full_ref(p: dict) -> str:
        ...
        cites = p.get("citation_count")
        cite_badge = (f' <span style="font-size:8pt; color:#555; background:#f0f4f8; '
                      f'padding:1px 5px; border-radius:3px;">📖 {cites} citation{"s" if cites != 1 else ""}</span>'
                      if cites is not None else "")
    ...
    all_pubs_html = """
    <div class="cv-section cv-section-allpubs-header">
...
def write_publication_sections() -> None:
    ...
    """
    # Remove stale pub_*.md files from previous runs (not the heading widget)
    for old in (REPO / "content" / "home").glob("pub_*.md"):
        old.unlink()

    # Write a heading-only widget as the "All Publications" anchor
    heading_widget = """+++
widget = "blank"
headless = true
active = true
...
"""
    ...
    print(f"  Wrote {heading_path.relative_to(REPO)}")
...
def write_menus(custom_sections: list[tuple[str, str]]) -> None:
    ...
    # All Papers + Patient Practice link
    all_entries = fixed + custom_entries + [
        ("All Papers", "#allpubs_heading", 190),
        ("My Practice", "/practice/", 195),
    ]
